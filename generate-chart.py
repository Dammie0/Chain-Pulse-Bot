...
def detect_patterns(swing_highs, swing_lows, closes, timestamps):
    patterns = []
    ...
    if len(sh) >= 2:
        ...
        if abs(h1["y"] - h2["y"]) / h1["y"] < 0.02:
            patterns.append({
                ...
                "description": "Double Top detected — bearish reversal likely"
            })

    # --- Double Bottom (Bullish) ---
    if len(sl) >= 2:
        l1, l2 = sl[-2], sl[-1]
        if abs(l1["y"] - l2["y"]) / l1["y"] < 0.02:
            patterns.append({
                "name": "Double Bottom",
                "signal": "BULLISH",
                "color": "#26a69a",
                ...
                "description": "Double Bottom detected — bullish reversal likely"
            })

    # --- Head and Shoulders (Bearish) ---
    if len(sh) >= 3:
        left, head, right = sh[-3], sh[-2], sh[-1]
        if (head["y"] > left["y"] and head["y"] > right["y"] and
                abs(left["y"] - right["y"]) / left["y"] < 0.03):
            patterns.append({
                "name": "Head & Shoulders",
                "signal": "BEARISH",
                "color": "#ef5350",
                ...
                "description": "Head & Shoulders — strong bearish reversal signal"
            })
    ...
    if len(sl) >= 3:
        ...
        if (head["y"] < left["y"] and head["y"] < right["y"] and
            ...
            patterns.append({
                ...
                "description": "Inverse H&S — strong bullish reversal signal"
            })

    # --- Ascending Triangle (Bullish) ---
    if len(sh) >= 2 and len(sl) >= 2:
        if (abs(sh[-1]["y"] - sh[-2]["y"]) / sh[-2]["y"] < 0.015 and
                sl[-1]["y"] > sl[-2]["y"]):
            patterns.append({
    ...
    if len(sh) >= 2 and len(sl) >= 2:
        if (abs(sl[-1]["y"] - sl[-2]["y"]) / sl[-2]["y"] < 0.015 and
                sh[-1]["y"] < sh[-2]["y"]):
            patterns.append({
    ...
    if len(sh) >= 2 and len(sl) >= 2:
        if sh[-1]["y"] > sh[-2]["y"] and sl[-1]["y"] > sl[-2]["y"]:
            patterns.append({
                ...
                "color": "#26a69a",
                "points": [],
                "description": "Higher Highs + Higher Lows — confirmed uptrend"
            })
    ...
    if len(sh) >= 2 and len(sl) >= 2:
        if sh[-1]["y"] < sh[-2]["y"] and sl[-1]["y"] < sl[-2]["y"]:
            patterns.append({
                ...
                "color": "#ef5350",
                "points": [],
                "description": "Lower Highs + Lower Lows — confirmed downtrend"
            })

    return patterns

def find_support_resistance(highs, lows, window=5):
    ...
    resistance = round(sorted(resistance_levels)[-1], 4) if resistance_levels else round(max(highs[-20:]), 4)
    support = round(sorted(support_levels)[0], 4) if support_levels else round(min(lows[-20:]), 4)
...
def generate_chart(symbol="BNB", interval="4h", limit=60):
    ...
    patterns = detect_patterns(swing_highs, swing_lows, closes, timestamps)
    ...
    for sh in swing_highs[-8:]:
        swing_high_dashes.append({"x": sh["x"] - 1800000, "y": sh["y"]})
        swing_high_dashes.append({"x": sh["x"] + 1800000, "y": sh["y"]})
        swing_high_dashes.append({"x": None, "y": None})

    # Swing low dashes
    swing_low_dashes = []
    for sl in swing_lows[-8:]:
        swing_low_dashes.append({"x": sl["x"] - 1800000, "y": sl["y"]})
        swing_low_dashes.append({"x": sl["x"] + 1800000, "y": sl["y"]})
        swing_low_dashes.append({"x": None, "y": None})
    ...
    if patterns:
        latest = patterns[-1]
        pattern_summary = f" | {latest['name']}: {latest['signal']}"
    ...
    datasets = [
        ...
        },
        {
            ...
            "data": fmt(ema9),
            "borderColor": "#F0B90B", "borderWidth": 1.5,
            "pointRadius": 0, "fill": False, "spanGaps": True, "yAxisID": "y"
        },
        {
            "type": "line", "label": "EMA 21",
            "data": fmt(ema21),
            "borderColor": "#2196F3", "borderWidth": 1.5,
            "pointRadius": 0, "fill": False, "spanGaps": True, "yAxisID": "y"
        },
        {
            "type": "line", "label": "BB Upper",
            "data": fmt(bb_upper),
            "borderColor": "rgba(156,39,176,0.7)", "borderWidth": 1,
            "borderDash": [4,4], "pointRadius": 0, "fill": False, "spanGaps": True, "yAxisID": "y"
        },
        {
            "type": "line", "label": "BB Middle",
            "data": fmt(bb_middle),
            "borderColor": "rgba(156,39,176,0.4)", "borderWidth": 1,
            "borderDash": [2,2], "pointRadius": 0, "fill": False, "spanGaps": True, "yAxisID": "y"
        },
        {
        ...
        {
            "type": "line", "label": f"Support ${support}",
            "data": support_line,
            "borderColor": "#00e676", "borderWidth": 1.5,
            "borderDash": [6,3], "pointRadius": 0, "fill": False, "yAxisID": "y"
        },
        {
            "type": "line", "label": f"Resistance ${resistance}",
            "data": resistance_line,
            "borderColor": "#ff1744", "borderWidth": 1.5,
            "borderDash": [6,3], "pointRadius": 0, "fill": False, "yAxisID": "y"
        },
        {
            "type": "line", "label": "Swing Highs",
            "data": [d if d["x"] is not None else {"x": None, "y": None} for d in swing_high_dashes],
            "borderColor": "#ff6d00", "borderWidth": 2,
            "borderDash": [8, 2], "pointRadius": 0, "fill": False,
            "spanGaps": False, "yAxisID": "y"
        },
        {
            "type": "line", "label": "Swing Lows",
            "data": [d if d["x"] is not None else {"x": None, "y": None} for d in swing_low_dashes],
            "borderColor": "#00e5ff", "borderWidth": 2,
            "borderDash": [8, 2], "pointRadius": 0, "fill": False,
            "spanGaps": False, "yAxisID": "y"
        },
        {
            "type": "line", "label": f"RSI({round(current_rsi,1)}) {rsi_signal}",
            "data": fmt(rsi),
            "borderColor": "#e040fb", "borderWidth": 1.5,
            "pointRadius": 0, "fill": False, "spanGaps": True, "yAxisID": "rsi"
        },
        {
            "type": "line", "label": "RSI 70",
            "data": rsi_70,
            "borderColor": "rgba(255,23,68,0.5)", "borderWidth": 1,
            "borderDash": [4,4], "pointRadius": 0, "fill": False, "yAxisID": "rsi"
        },
        {
            "type": "line", "label": "RSI 30",
            "data": rsi_30,
            "borderColor": "rgba(0,230,118,0.5)", "borderWidth": 1,
            "borderDash": [4,4], "pointRadius": 0, "fill": False, "yAxisID": "rsi"
        }
    ] + pattern_datasets
    ...
    chart_config = {
        ...
        "options": {
            "plugins": {
                "title": {
                    "display": True,
                    "text": f"{symbol}/USDT {interval} | ${current_price} | {trend} | RSI:{round(current_rsi,1)}{pattern_summary}",
                    "color": "#ffffff",
                    "font": {"size": 13}
                },
